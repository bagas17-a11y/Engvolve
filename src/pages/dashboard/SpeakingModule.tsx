...
export default function SpeakingModule() {
  ...
  const exportResult = () => {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Speaking Feedback — Engvolve</title>
...
<body><h1>IELTS Speaking Feedback</h1><p>Engvolve &mdash; ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p>
        ...
        {feedback && (
          <div className="space-y-4">
            ...
            {feedback.overallBand && (
              <button
                onClick={() => {
                  const text = `I just scored Band ${feedback.overallBand!.toFixed(1)} on IELTS Speaking on Engvolve! 🎯`;
